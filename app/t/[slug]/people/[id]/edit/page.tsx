...
import { getRoleIn, getTreeById, requireTreeSelfPerson } from "@/lib/tree-context";
...
export default async function EditPersonPage({
  params,
}: PageProps<"/t/[slug]/people/[id]/edit">) {
  const { slug, id } = await params;
  const { tree, profile, type } = await requireTreeSelfPerson(slug);

  const supabase = await createClient();
  const { data: person } = await supabase
    .from("tree_people")
    .select(
      "id, home_tree_id, is_home, first_name, middle_name, preferred_name, maiden_name, last_name, date_of_birth, date_of_birth_precision, place_id_birth, city_of_birth, country_of_birth, is_deceased, date_of_death, date_of_death_precision, place_id_death, place_of_death, sex, lineage_type, photo_path, photo_crop, owner_user_id, created_by",
    )
    .eq("tree_id", tree.id)
    .eq("id", id)
    .maybeSingle();

  if (!person?.id || !person.last_name || !person.home_tree_id || !person.owner_user_id || !person.created_by) {
    notFound();
  }
  ...
  if (!canEdit) redirect(treeFocusHref(slug, personId));
  ...
  const connections: ExistingConnection[] = (rels ?? []).flatMap((r) => {
    if (!r.id || !r.from_person || !r.to_person || !r.type || !r.created_by) return [];
    ...
    return [
      {
        ...
        canRemove: canEditConnection(
          { from_person: r.from_person, to_person: r.to_person, created_by: r.created_by },
          treeViewer,
        ),
      },
    ];
  ...
  return (
    <main className="mx-auto flex w-full max-w-2xl flex-1 flex-col gap-6 px-4 py-10">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button
          nativeButton={false}
          render={<Link href={treeHref(slug)} />}
          size="sm"
          variant="outline"
        >
          Back to tree
      ...
      {type.connections === "none" ? (
        ...
        <EditConnections
          treeId={tree.id}
          personId={personId}
          personName={personDisplayName({ ...person, last_name: person.last_name })}
          personPartners={
