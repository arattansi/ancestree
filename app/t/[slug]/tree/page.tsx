...
export default async function TreePage({ params }: PageProps<"/t/[slug]/tree">) {
  ...
  if (!profile.self_person_id) redirect(onboardingHref(slug));
  ...
  const { data: placed } = await supabase
    ...
    .eq("status", "active")
    .maybeSingle();
  if (!placed) redirect(onboardingHref(slug));
  ...
  if (people.length === 0) {
    return (
      <main className="flex flex-1 flex-col items-center justify-center gap-4 px-6 py-24 text-center">
        ...
        <Button nativeButton={false} render={<Link href={onboardingHref(slug)} />}>
          Add yourself
        </Button>
      </main>
    );
