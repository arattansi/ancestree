-- Step 11.3 — Blocking approval modal: server side
--
-- `add_people_with_connections` gains a 4th argument, `p_suggestions`: the
-- implied connections the adder resolved in the blocking modal. Each row is
-- persisted to `connection_suggestions`; an `accepted` spouse/parent suggestion
-- also creates its relationship edge — all inside the one transaction that
-- creates the new person and their base edges.
--
-- Also adds:
--   * a guard rejecting a pair that is both a parent-child and a spouse edge
--     (shared by Step 11.4's multi-connection flow), and
--   * `resolve_connection_suggestion()` for resolving a still-pending
--     suggestion later from a person's detail panel.
--
-- Down: recreate the 3-arg function from migration 20260830210000 and
--   `drop function public.add_people_with_connections(jsonb, jsonb, int, jsonb);
--    drop function public.resolve_connection_suggestion(uuid, text);`

drop function if exists public.add_people_with_connections(jsonb, jsonb, int);

create function public.add_people_with_connections(
  p_people jsonb,
...
-- ---------------------------------------------------------------------------
-- resolve_connection_suggestion — resolve a still-pending suggestion later,
-- from a person's detail panel. Creator or admin only (RLS also enforces it).
-- ---------------------------------------------------------------------------
