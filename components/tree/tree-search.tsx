...
export function TreeSearch({ people, filter, onFilterChange, onPick }: Props) {
  ...
  /** Below `sm` the card is collapsed to an icon so it can't cover the canvas
   *  actions in the opposite corner. */
  ...
  const clear = () => {
    onFilterChange({ text: "", country: "", birthDecade: "", living: "any" });
    setOpen(false);
    setMobileOpen(false);
  };
  ...
  return (
    <>
      <button
        ...
        className={cn(
          "flex size-9 items-center justify-center rounded-full border border-border bg-card text-muted-foreground shadow-md sm:hidden",
          mobileOpen && "hidden",
        )}
      ...
      <div
        className={cn(
          "relative z-10 w-[calc(100vw-2rem)] max-w-72 rounded-xl border border-border bg-card p-3 shadow-md sm:w-72",
        ...
        <div className="flex items-center gap-2">
          ...
          {active || mobileOpen ? (
            <button
              ...
              aria-label={active ? "Clear search" : "Close search"}
            >
              <X className="size-4" />
            </button>
          ) : null}
