...
describe("ancestral lands", () => {
  const toronto =
    "Ancestral lands of Anishinaabe, including the Mississaugas of the Credit, Haudenosaunee, and Huron-Wendat (Wyandot) territory";

  it("keeps the family's own words for the place of birth, trimmed", () => {
    const payload = toPersonPayload(
      person({ ancestral_lands_birth: `  ${toronto}  ` }),
    );
    expect(payload.ancestral_lands_birth).toBe(toronto);
  });

  it("stores nothing for an empty box, so the card looks the names up instead", () => {
    const payload = toPersonPayload(person({ ancestral_lands_birth: "   " }));
    expect(payload.ancestral_lands_birth).toBeNull();
  });

  it("keeps words about a place of death only while there is one", () => {
    const deceased = person({
      is_deceased: true,
      place_id_death: 6167865,
      ancestral_lands_death: toronto,
    });
    expect(toPersonPayload(deceased).ancestral_lands_death).toBe(toronto);
    expect(
      toPersonPayload({ ...deceased, is_deceased: false }).ancestral_lands_death,
    ).toBeNull();
    expect(
      toPersonPayload({ ...deceased, place_id_death: null }).ancestral_lands_death,
    ).toBeNull();
  });

  it("refuses more than 300 characters", () => {
    const result = personSchema.safeParse(
      person({ ancestral_lands_birth: "x".repeat(301) }),
    );
    expect(result.success).toBe(false);
