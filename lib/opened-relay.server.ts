 ...
 * the tree it was answered into and when it was filed (Step 41.5). RLS
 * shows an ask only to the member it went to, so for anyone else — or once
 * it's gone — this is `null`.
 */
export async function loadOpenedRelay(relayId: string): Promise<OpenedRelay | null> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("invite_relays")
    .select("status, first_name, last_name, created_at, trees(name)")
    .eq("id", relayId)
    .maybeSingle();
  if (!data) return null;
