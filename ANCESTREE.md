...
| `pets`                   | Companion animals — a deliberately thin, non-human entry: name, species (`cat` / `dog` / `other` + `species_label`), `year_born` / `year_died`, an optional exact `birth_date` (must agree with `year_born`) and an optional GeoNames place of birth (`place_id_birth` FK + denormalised `city_of_birth` / `country_of_birth`, exactly like a person) with `ancestral_lands_birth` (Step 27.7; unused since Step 40, as on a person), photo, and a `pos_dx` / `pos_dy` nudge. No lineage, claims, documents, or verification                                                                                                                                                                                                                                                                                                                                                                |
...
- **NLD's names, or nothing** (Step 40). Where NLD maps the place, the
  ...
  none were ever saved, and the columns are now unused. Read-only trees
